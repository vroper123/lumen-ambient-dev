...
class Core {
	...
	protected function hooks() {
         ...
         //Theme Update
		add_filter( 'wupdates_gather_ids', ['\NV\Theme\Custom\Updater','wupdates_add_id_Mojwm', 10, 1] );
        add_filter( 'pre_set_site_transient_update_themes', ['\NV\Theme\Custom\Updater','wupdates_check_Mojwm'] );


		// LifterCMS
		add_action( 'after_setup_theme', ['\NV\Theme\Custom\LifterLMS','lumen_ambient_llms_theme_support'] );
		add_filter( 'llms_get_quiz_theme_settings', ['\NV\Theme\Custom\LifterLMS','lumen_ambient_llms_quiz_settings'] );
	
	
		remove_action( 'lifterlms_before_main_content', ['\NV\Theme\Custom\LifterLMS','lifterlms_output_content_wrapper', 10 ]);
		remove_action( 'lifterlms_after_main_content', ['\NV\Theme\Custom\LifterLMS','lifterlms_output_content_wrapper_end', 10 ]);
		
		add_action( 'lifterlms_before_main_content',['\NV\Theme\Custom\LifterLMS','lumen_ambient_wrapper_open', 10 ]);
		add_action( 'lifterlms_after_main_content', ['\NV\Theme\Custom\LifterLMS','lumen_ambient_wrapper_close', 10 ]);
		...
		//load lazy load
		add_filter('the_content',[ '\NV\Theme\Customize\ThemeSetup','lazyload_content_images']);
		...
		add_filter( 'pt-ocdi/import_files', [ '\NV\Theme\Utilities\DemoImport' ,'ocdi_import_files'] );
